# Code made by JUNSIK HWANG: https://jsideas.net/snapshot_ensemble/
...
class CosineAnnealingLR_with_Restart(_LRScheduler):
    ...
    def step(self, epoch=None):
        ...
        if self.current_epoch == self.Te:
            print(f"restart at epoch {self.last_epoch + 1}")
            
            if self.take_snapshot:
                torch.save({
                    'epoch': self.T_max,
                    'state_dict': self.model.state_dict()
                }, self.out_dir + f'snapshot_e_{self.T_max:03d}.pth.tar')
